...
graph_tab_07 <- full%>% dplyr::select(groups_ICW07, groups, sibs) %>% 
  drop_na(groups_ICW07, sibs)%>%
  group_by(groups)%>% 
  count(groups_ICW07, groups, sibs) %>%  
...
################################################################################
graph_tab_BRQ11 <- full%>% dplyr::select(groups_BRQ11, groups, sibs) %>% 
  drop_na(groups_BRQ11, sibs)%>%
  group_by(groups)%>% 
  count(groups_BRQ11, groups, sibs) %>%  
  mutate(prop = prop.table(n)) %>%
  group_by(groups, sibs) %>% 
  mutate(sample_size = sum(n)) %>% 
...
################################################################################
graph_tab_08 <- full%>% dplyr::select(groups_ICW08, groups, sibs) %>% 
  drop_na(groups_ICW08, sibs)%>%
  group_by(groups)%>% 
  count(groups_ICW08, groups, sibs) %>%  
  mutate(prop = prop.table(n)) %>%
  group_by(groups, sibs) %>% 
  mutate(sample_size = sum(n)) %>% 
...
graph_tab_02 <- full%>% dplyr::select(groups_ICW02, groups, sibs) %>% 
  drop_na(groups_ICW02, sibs)%>%
  group_by(groups)%>% 
  count(groups_ICW02, groups, sibs) %>%  
  mutate(prop = prop.table(n)) %>%
  group_by(groups, sibs) %>% 
  mutate(sample_size = sum(n)) %>% 
...
g <- ggarrange(figure_sibs,gg_prop_eiw, gg_prop_nc, gg_prop_ed,gg_prop_w,
               nrow = 5,ncol = 1, common.legend = TRUE, legend = "bottom")

pdf("overlap-sibs.pdf", width = 8.5, height = 11)
a <- annotate_figure(g,
                     top=textGrob("Outcome by rhinitis phenotypes stratified by presence of older siblings",gp=gpar(fontsize=14,font=1)),
                     left=textGrob("Percent with ever phenotype",gp=gpar(fontsize=13,font=1), rot = 90))
a
dev.off()
