...
dta_overall <- read.csv("model-output.csv") %>% dplyr::select(-1) %>%
  dplyr::select(outcome, exposure, OR ,X95_CI_LOW, X95_CI_HIGH)%>%
  mutate(group = "overall")%>% filter(outcome == "MDAsthma")


#--------------------------------------------------------------------------------
# data stratified
setwd("~/Desktop/projects/Mattlab/rhinitis/model-output/logistic")
sex_w <- read.csv("atopy_strata_MDAsthma.csv") %>% dplyr::select(-1) %>%
  mutate(outcome = "MDAsthma")%>%
...
dta$exposure <- factor(dta$exposure, levels = rev(c("A","B","C","D", "E")))
...
dta <- dta %>% filter(!(exposure == "E" & group == "atopic"))
...
pdf("stratified-atopy-asthma.pdf", width = 6, height = 3)
p
dev.off()
