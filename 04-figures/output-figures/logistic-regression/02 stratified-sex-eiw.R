...
# data stratified
setwd("~/Desktop/projects/Mattlab/rhinitis/model-output/logistic")
at_eiw <- read.csv("ICW07_sex-rev.csv") %>% dplyr::select(-1) %>%
  mutate(outcome = "groups_ICW07")%>%
...
dta <- dta %>% filter(!(exposure == "A" & group == "boys"))
...
pdf("stratified-sex-eiw.pdf", width = 6, height = 3)
p
dev.off()
